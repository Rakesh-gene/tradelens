import { formatMeasurementValue, formatScore } from '../utils/formatters.js'
...
export default function ScoreBreakdown({ scores, components }) {
  return <section aria-label="Score breakdown">
    ...
    {components && Object.keys(components).length > 0 && <details><summary>Scoring components</summary><dl className="measurement-grid">{Object.entries(components).map(([name, value]) => <div key={name}><dt>{name}</dt><dd>{formatMeasurementValue(value)}</dd></div>)}</dl></details>}
