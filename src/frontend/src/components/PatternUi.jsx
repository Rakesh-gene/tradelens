...
export function SetupCard({ setup, onNavigate }) {
  return <article className="setup-card">
    ...
    <DecisionSummary decision={setup.decision} compact />
    ...
    {!setup.decision && setup.bestFit && <div className="best-fit-summary"><p>{setup.bestFit.strengths?.[0] || 'No leading strength identified yet.'}</p>{setup.bestFit.cautions?.[0] && <p className="muted-copy">Watch: {setup.bestFit.cautions[0]}</p>}</div>}
...
export function PageIntro({ eyebrow, title, description, date }) {
  return <header className="page-intro"><div>{eyebrow && <p className="eyebrow">{eyebrow}</p>}<h1>{title}</h1>{description && <p>{description}</p>}</div>{date && <div className="date-chip"><span>Data as of</span><strong>{formatMarketDate(date)}</strong></div>}</header>
}
