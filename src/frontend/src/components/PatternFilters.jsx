...
export default function PatternFilters({ filters, facets, onChange, onApply, onClear, drawer = false, open = false, onClose }) {
  ...
  const form = <form className="filter-bar" onSubmit={onApply}>
    ...
    <label>Lifecycle state<select className="select-control" value={filters.state} onChange={change('state')}><option value="">All states</option>{SETUP_STATES.map((value) => <option key={value}>{value}</option>)}</select></label>
    <label>Pattern family<select className="select-control" value={filters.patternClass} onChange={change('patternClass')}><option value="">All families</option>{PATTERN_CLASSES.map((value) => <option key={value}>{value}</option>)}</select></label>
    <label>Pattern type<select className="select-control" value={filters.patternType} onChange={change('patternType')}><option value="">All types</option>{patternTypes.map((value) => <option key={value} value={value}>{value}{counts.has(value) ? ` (${counts.get(value)})` : ''}</option>)}</select></label>
